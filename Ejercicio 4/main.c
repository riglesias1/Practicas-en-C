...
void entrada(struct lote *ref, int i)
{
    float x;
    printf("\nEscriba los metro del Frente del Lote #%d: ",i);
    scanf("%f", &x); // i obtiene el valor entrante de consola
    (*ref).frente = x;
    printf("Ahora los metro del Fondo del Lote #%d: ",i);
    scanf("%f", &x); // i obtiene el valor entrante de consola
    (*ref).fondo = x; // Igualo p al valor de i
}
...
int main()
{
    ...
    float sup[LMX], x, *p=vec; // Superficies de Lotes
    int mfr,mfo,msu; // Usado como referencia

    for (int i=0; i<LMX; i++){ entrada(&vec[i], i); } // Entrada de datos
